...
а `rating` принимает только значения от 1 до 5. Для создания отзыва поле `consent` обязательно
должно иметь значение `true`: так API независимо от frontend проверяет, что посетитель подтвердил
согласие на обработку и публикацию данных отзыва. Значение согласия не сохраняется в таблице
отзывов.
...
`422 Unprocessable Entity`. Отсутствующее поле `consent` и значение `false` также возвращают
`422 Unprocessable Entity`:

```json
{
  "message": "Для публикации отзыва необходимо согласие на обработку данных"
}
```
